...
void MCP4X::write(unsigned int data)
{
	...
	if (MCP4X_PORT_WRITE)
		PORTB &= ~(1 << 0); // Clear PORTB pin 2 = arduino pin 10
	else
		digitalWrite(ss_pin, LOW);

	// Send the command and data bits
	// Serial.println("At MCP4x::write: ");
	// Serial.println((data & 0xf000) >> 12);
	// Serial.println(data & 0xfff);
	SPI.transfer((data & 0xff00) >> 8);
	SPI.transfer(data & 0xff);

	// Return chip select to high
	if (MCP4X_PORT_WRITE)
		PORTB |= (1 << 0); // set PORTB pin 2 = arduino pin 10
	else
