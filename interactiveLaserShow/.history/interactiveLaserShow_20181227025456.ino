    ...
    else
    {
        ...
        if (Serial.available() > 0)
        {
            ...
            if ((Serial.available() + bufLength) >= 4)
            {
                ...
                tmpInput = popFromSerial() << 8;
                tmpInput += popFromSerial();
                ...
                laser.sendto(posX & 0x7fff, posY & 0xfff);
                // Serial.write((posX & 0xf00) >> 8);
                // Serial.write(posX & 0xff);
                Serial.printNumber((unsigned long)posX, 1);
                Serial.write(',');
                Serial.printNumber((unsigned long)posY, 1);
                // Serial.write((posY & 0xf00) >> 8);
                // Serial.write(posY & 0xff);
                Serial.write('\n');
            }
            waitCnt = 0;
        }
