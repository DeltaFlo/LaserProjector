...
{
  // for uno
  // dac.init(MCP4X_4922, 2500, 2500, 10, 7, 1);
  // for mega
  dac.init(MCP4X_4922, 2500, 2500, 23, 16, 1);
...
{
  // Serial.print("At Lasor:: sendtoDAC");
  // Serial.print("x: ");
  // Serial.print(x);
  // Serial.print("y: ");
  // Serial.println(y);
...
void Laser::sendto(long xpos, long ypos)
{
  ...
  if (_enable3D)
  {
    ...
    xpos = ((_zDist * (long)p.x) / (_zDist + (long)p.z)) + 2048; //ここ計算式違くない？
...
{
  // Serial.print("At Lasor:: sendtoRaw");
  // Serial.print("x: ");
  // Serial.print(xNew);
  // Serial.print("y: ");
  // Serial.println(yNew);
